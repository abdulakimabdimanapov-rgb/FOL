...
# Phase 6: LLM calls go through the canonical bridge (legacy adapter kept
# for direct-import consumers until fully retired).
...
def _process_year(
    ...
    Uses the shared LLM module (reads API key from env).
    """
    ...
    for batch_idx, batch in enumerate(batches):
        ...
        year_block = f"Year: {year} | Batch {batch_idx + 1}/{len(batches)}\n\n"
        text_block = year_block + "\n---\n".join(text_lines)
        ...
        # Use the canonical LLM bridge (single routing abstraction)
        from orchestrator.llm_bridge import llm_call_json
        result = llm_call_json(full_prompt, "", max_tokens=_MAX_LLM_TOKENS)
