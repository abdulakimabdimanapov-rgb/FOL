...
# Obsidian sync debounce: max 1 sync per 30s to avoid excessive HTTP calls
_last_obsidian_sync_time: float = 0.0
_OBSIDIAN_SYNC_INTERVAL: float = 30.0
_obsidian_connection_cache: bool | None = None
_obsidian_connection_cache_time: float = 0.0
_OBSIDIAN_CACHE_TTL: float = 30.0


def _check_obsidian_cached() -> bool:
    """Check Obsidian connection with 30s cache."""
    import time
    now = time.time()
    global _obsidian_connection_cache, _obsidian_connection_cache_time
    if _obsidian_connection_cache is not None and (now - _obsidian_connection_cache_time) < _OBSIDIAN_CACHE_TTL:
        return _obsidian_connection_cache
    try:
        from obsidian.client import check_connection
        _obsidian_connection_cache = check_connection()
        _obsidian_connection_cache_time = now
        return _obsidian_connection_cache
    except Exception:
        _obsidian_connection_cache = False
        _obsidian_connection_cache_time = now
        return False


def _sync_to_obsidian(date_str: str, category: str, summary: str, source: str) -> None:
    """Sync a single episodic event to Obsidian Episodic/events.md.

    Never raises — silently skips if Obsidian is unavailable.
    Debounced: max 1 sync per 30s. Connection cached for 30s.
    """
    import time
    global _last_obsidian_sync_time
    now = time.time()
    if (now - _last_obsidian_sync_time) < _OBSIDIAN_SYNC_INTERVAL:
        return  # Debounce
    _last_obsidian_sync_time = now

    if not _check_obsidian_cached():
        return

    try:
        from obsidian.client import read_note, write_note

        vault_path = "Episodic/events"
        line = f"- [{date_str}] **{category}** — {summary} _(via {source})_"

        try:
            existing = read_note(vault_path)
            new_content = existing + "\n" + line if existing else (
                "# Episodic Events\nAuto-synced from FOL.\n\n" + line
            )
        except Exception:
            new_content = "# Episodic Events\nAuto-synced from FOL.\n\n" + line + "\n"

        # Keep max 200 lines to prevent unbounded growth
        MAX_LINES = 200
        ...
        logger.debug("Synced event to Obsidian Episodic/events")
    except Exception:
        pass  # Silent fallback — Obsidian may not be running
...
def append_event(
    ...
    1. ~/.secondself/episodic.md  (legacy, for build_system_prompt)
    2. output/episodic.md          (local copy)
    3. Obsidian Episodic/events.md (living memory)
    ...
    try:
        ...
        _append_line(EPISODIC_PATH, line)
        _append_line(OUTPUT_PATH, line)

        # Sync to Obsidian (living memory) — silent fallback if unavailable
        _sync_to_obsidian(date_str, category, safe_summary, safe_source)
...
def get_recent_events(n: int = 10) -> list[dict[str, Any]]:
    ...
    Most recent first.
    """
    lines = _read_event_lines(EPISODIC_PATH)
