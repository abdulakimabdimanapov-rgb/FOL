...
def test_extract_vocabulary_markers() -> None:
    with patch("analyze.voice_analyzer.llm_call_json", return_value={"vocabulary_markers": ["word1", "word2"]}):
...
def test_extract_vocabulary_markers_caps_at_15() -> None:
    ...
    with patch("analyze.voice_analyzer.llm_call_json", return_value={"vocabulary_markers": markers}):
        result = va._extract_vocabulary_markers("text block")
    assert len(result) == 15


def test_extract_vocabulary_markers_empty_response() -> None:
    with patch("analyze.voice_analyzer.llm_call_json", return_value={}):
        result = va._extract_vocabulary_markers("text block")
    assert result == []


def test_extract_tone_descriptor() -> None:
    with patch("analyze.voice_analyzer.llm_call_json", return_value={"tone_descriptor": "casual"}):
