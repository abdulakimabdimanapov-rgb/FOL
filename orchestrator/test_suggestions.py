...
from server import (
    ...
    _conversation_history, broadcast_event, event_clients,
)
from suggestion_engine import (
    profile_trigger, pattern_trigger, _parse_suggestions, _call_llm_sync,
...
class TestProfileTrigger:
    @patch("suggestion_engine._call_llm_sync")
...
class TestResetClearsState:
    ...
    async def test_reset_clears_conversation_and_profile(self, client):
        import server
        server._conversation_history.extend([
        ...
        assert resp.status_code == 200
        # /reset rebinds the module global, so assert through the module
        # attribute (a direct import binding would be stale).
        assert len(server._conversation_history) == 0
