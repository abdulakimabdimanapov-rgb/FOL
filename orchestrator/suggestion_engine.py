...
SuggestionEngine — generates proactive suggestions for the FOL twin.
...
def _call_llm_sync(system_prompt: str, user_content) -> str:
    ...
    try:
        ...
        return llm_completion_sync(messages=messages, system=system_prompt, max_tokens=2048)
...
def profile_trigger(profile: dict) -> list[dict]:
    ...
    text = _call_llm_sync(prompt, "Generate suggestions for this user.")
...
def pattern_trigger(conversation_history: list[dict], profile: dict | None = None) -> list[dict]:
    ...
    text = _call_llm_sync(prompt, "Analyze for patterns.")
    suggestions = _parse_suggestions(text)
...
def ambient_tick(
    ...
    text = _call_llm_sync(prompt, user_content)
    suggestions = _parse_suggestions(text)
