...
class TestChatSSEEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_chat_sse_filters_internal_tool_events(self, client):
        ...
        async def mock_streaming(task, max_steps=15):
            ...
            yield ("activity", {"category": "computer", "label": "Работаю в браузере…"})
        ...
        # Only coarse, user-safe status reaches the UI.
        activity_events = [e for e in collected_events if e[0] == "activity"]
        assert len(activity_events) == 1
        assert "browser_goto" not in json.dumps(activity_events[0][1])
        assert activity_events[0][1]["label"] == "Работаю в браузере…"
...
class TestErrorHandling:
    ...
    async def test_anthropic_429_in_stream(self, client):
        ...
        # The Response Formatter localizes known error messages (RU/EN) and
        # drops the literal HTTP code, so assert on the localized meaning
        # ("429" stays when a raw message passes through untouched).
        msg = error_event[1]["message"].lower()
        assert any(k in msg for k in ("429", "request", "запрос")), error_event[1]["message"]
