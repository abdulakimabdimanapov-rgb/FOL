...
### Fixed (2026-08-16 — Stardance frictionless pass)

- **P0: репозиторий не запускался** — две половины проекта (`Fol/` backend и
  `SecondSelf/` FOL-core + SwiftUI) были разнесены, а код и документация
  (`MERGED_README.md`, `run_all.sh`, `parent.parent / "fol"`) ожидали единый
  корень. Слито в один корень: `fol/`, `orchestrator/`, `SecondSelf/`,
  `tests/` и т.д. — теперь импорты и все сервисы работают.
- **`fol_command` handler binding** (`orchestrator/server.py`): контекст
  хэндлеров захватывал `call_fol_api` по ссылке при импорте — `patch()` и
  живой вызов не совпадали. Теперь резолвится через модульный глобал в
  момент вызова (в проде поведение то же, тесты видят патч).
- **«Empty command» для fol_command** — валидация схемы отклоняла пустую/
  отсутствующую `command` раньше хэндлера, задокументированное сообщение
  было недостижимо. Возвращается `{"error": "Empty command"}`.
- **E2E false positive** (`scripts/e2e_check.py`): проверка логов помечала
  `litellm.APIError` (нотисы фолбэк-цепочки провайдеров — штатная деградация)
  как «ERROR». Теперь ловятся только реальные traceback и ERROR-записи.
- **Swift build после переезда** — `.build/` приехал со старого пути и
  ломался на кэше `SwiftShims`; чистый пересбор работает.
- **`build-app.sh` подпись** — `xattr -cr` перед codesign, чтобы сборка
  `.app` проходила без ручных шагов.
- **Секреты** — `.env.bak`/`.env.template` содержали реальные ключи, но не
  были в `.gitignore`. `.env*` игнорируются, безопасные шаблоны (пустые
  ключи) коммитятся; репозиторий просканирован — чисто.
