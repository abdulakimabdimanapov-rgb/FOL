...
def close_app(name: str) -> dict:
    """Quit a macOS application by name via AppleScript."""
    safe = _escape_as_string(name)
    script = f'''
    ...
    return {"status": "error", "action": "close_app", "app": name,
            "error": result.stderr.strip() or "App not running or cannot be quit"}


def drag(x1: int, y1: int, x2: int, y2: int, duration: float = 0.3) -> dict:
    """Drag the mouse from (x1, y1) to (x2, y2) with a smooth motion."""
    import pyautogui
...
def clipboard_get() -> dict:
    ...
    return {"status": "ok", "action": "clipboard_get", "text": result.stdout}


def clipboard_set(text: str) -> dict:
    """Set the clipboard text via pbcopy."""
    result = subprocess.run(
        ["pbcopy"], input=text, capture_output=True, text=True, timeout=5
    )
    if result.returncode == 0:
        return {"status": "ok", "action": "clipboard_set", "length": len(text)}
    return {"status": "error", "action": "clipboard_set", "error": result.stderr.strip()}


def notify(title: str, message: str = "") -> dict:
    """Show a macOS notification banner via osascript (Notification Center)."""
    safe_title = _escape_as_string(title)
    safe_message = _escape_as_string(message)
    script = f'''
    ...
    if result.returncode == 0:
        return {"status": "ok", "action": "notify", "title": title}
    return {"status": "error", "action": "notify", "error": result.stderr.strip() or "Notification failed"}
...
def _escape_as_string(text: str) -> str:
    r"""Escape text for embedding in an AppleScript string literal.

    AppleScript uses " for strings. Inside a string, escape:
      " → \" (backslash-quote)
      \ → \\ (backslash-backslash)
    """
...
def safari_goto(url: str) -> dict:
    """Navigate Safari to a URL."""
    safe_url = _escape_as_string(url)
    script = f'''
    ...
    if result.returncode == 0:
        return {"status": "ok", "action": "safari_goto", "url": url}
    return {"status": "error", "error": result.stderr.strip() or "Safari failed to navigate"}


def safari_js(javascript: str) -> dict:
    """Execute JavaScript in Safari's current tab."""
    escaped = _escape_as_string(javascript)
    script = f'''
    tell application "Safari"
        if (count of windows) = 0 then return "No open window"
        set resultText to do JavaScript "{escaped}" in current tab of front window
        return resultText
    end tell
    '''
    result = subprocess.run(
...
def safari_get_text() -> dict:
    """Get the visible text content of the current Safari page."""
    script = '''
    tell application "Safari"
        if (count of windows) = 0 then return ""
        set pageText to text of current tab of front window
        return pageText
    end tell
    '''
    result = subprocess.run(
    ...
    if result.returncode == 0:
        return {"status": "ok", "text": result.stdout.strip()[:5000]}
    return {"status": "error", "error": result.stderr.strip()}
...
TOOLS = {
    ...
    "/tool/close_app": lambda body: close_app(body["name"]),
    "/tool/drag": lambda body: drag(
        body.get("x1", 0), body.get("y1", 0),
        body.get("x2", 0), body.get("y2", 0),
        body.get("duration", 0.3),
    ),
    "/tool/clipboard_get": lambda body: clipboard_get(),
    "/tool/clipboard_set": lambda body: clipboard_set(body.get("text", "")),
    "/tool/notify": lambda body: notify(body.get("title", "FOL"), body.get("message", "")),
    "/tool/move": lambda body: move_mouse(body["x"], body["y"]),
    "/tool/screen_size": lambda body: get_screen_size(),
    "/safari/goto": lambda body: safari_goto(body["url"]),
    "/safari/js": lambda body: safari_js(body["javascript"]),
    "/safari/url": lambda body: safari_get_url(),
    "/safari/text": lambda body: safari_get_text(),
...
class AgentHandler(BaseHTTPRequestHandler):
    ...
    def _serve_viewer(self):
        ...
        html = """<!DOCTYPE html>
<html><head><title>FOL — Live View</title>
<style>
