...
<<<<<<< HEAD
3. **Прочитай DESIGN.md** — дизайн-система (цвета, шрифты, анимации).
4. **Прочитай `orchestrator/server.py`** — главный файл: LLM + tool loop + SSE.
5. **Посмотри `fol-app/`** — SwiftUI UI слой.
=======
...
- **Документируй.** Обновляй FOL_UPGRADE.md после значимых изменений.
