...
/// Manages the notch overlay with a pure NSPanel (no DynamicNotchKit dependency).
...
final class NotchOverlayController: NSObject {
    ...
    // `nonisolated deinit` intentionally omitted for now.
    // NotchOverlayController lives for the entire app session (held by AppDelegate),
    // so deinit cleanup is not required in practice.
    // Event monitors and windows are cleaned up on process termination by macOS.

    override init() {
        let viewModel = ChatViewModel()
        self.chatViewModel = viewModel

        self.notchPanel = NotchPanel(viewModel: viewModel)
    ...
    func togglePanel() {
        ...
        switch currentStage {
        case 0:
            // Compact → status mini: expand notch + set stage 1
            chatViewModel.expansionStage = 1
            Task {
                await notchPanel.expand()
                isExpanded = true
            }
        case 1:
            ...
            makeNotchPanelKey()
            // Notify AppDelegate for status logging
            NotificationCenter.default.post(name: .secondSelfChatOpened, object: nil)
        default:
            // Full chat → compact
            collapse()
        }
    ...
    /// LOCAL monitor: sees clicks delivered to OUR app (i.e. clicks on the notch panel).
    private func installLocalClickMonitor() {
        localClickMonitor = NSEvent.addLocalMonitorForEvents(
            ...
            case 2:
                ...
                // BUT: after Cmd+Tab the panel may have lost keyWindow status.
                // Reclaim it so the text field can receive keyboard input.
                if !self.notchPanel.isKeyWindow {
                    self.makeNotchPanelKey()
                }
                return event
    ...
    /// Compute the notch frame using screen geometry:
    ...
    /// Makes the notch panel the key window so text fields can receive focus.
    ...
    /// Read the actual bottom edge of the notch panel window.
    private func notchPanelBottomY() -> CGFloat {
        return notchPanel.frame.minY
    }
