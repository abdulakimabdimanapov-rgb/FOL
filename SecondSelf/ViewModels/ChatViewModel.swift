...
final class ChatViewModel: ObservableObject {
    ...
    private func handleActivity(data: String) {
        ...
        // Show the VNC feed when the agent works on the computer itself.
        if category == "computer" && !showVNCFeed {
            userDismissedVNC = false
            showVNCFeed = true
        }

        // Update the VNC bottom bar with a generic human label.
        if !label.isEmpty {
            currentToolAction = label
        }
    }
    ...
    /// Check if any STT service is available (OpenAI Whisper or ElevenLabs).
    /// The mic button is visible if at least one speech-to-text API key is configured.
    ...
    /// Prefers OpenAI Whisper; falls back to ElevenLabs Scribe.
