...
import type { FOLProfile } from "@/lib/api";
...
interface BuildingScreenProps {
  ...
  sessionId: string;
  onComplete: () => void;
  onProfile: (profile: FOLProfile) => void;
}
...
export default function BuildingScreen({
  ...
  const profileRef = useRef<FOLProfile | null>(null);
  ...
  // Call /onboard on mount (once only). The latest onProfile callback is kept
  // in a ref — updated inside an effect, never during render (react-hooks/refs).
