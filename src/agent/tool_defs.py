...
# ---------------------------------------------------------------------------
# Canonical registry adapter (Phase 5) — single source of truth
# ---------------------------------------------------------------------------
# The 14 productivity tools below are defined once, canonically, in
# fol/modules/tools/orchestrator_tools.py. This adapter lets the web-tier
# agent consume the canonical schemas instead of maintaining a second copy.
# The legacy TOOL_DEFINITIONS list is preserved for backward compatibility;
# new code should prefer canonical_tool_definitions().


def canonical_tool_definitions() -> list[dict[str, Any]]:
    """Anthropic-format tool schemas derived from the canonical ToolRegistry.

    Falls back to the legacy TOOL_DEFINITIONS when the canonical registry is
    unavailable (e.g. fol/ not importable) — never raises.
    """
    try:
        import os
        import sys
        ...
        return [spec.to_anthropic() for spec in registry.filter(category="productivity")]
    except Exception:
