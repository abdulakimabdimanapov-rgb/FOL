...
final class NotchOverlayController: NSObject {
    ...
    private func handleTwinStateChange(_ twinState: TwinState) {
        switch twinState {
        case .thinking:
            // A user message was just sent — remember it so we can open the
            // full chat when the agent starts working (or the voice loop does).
            break

        case .working:
            // Auto-expand is ON by default (setting only turns it OFF).
            let autoExpand = UserDefaults.standard.object(forKey: "autoExpandOnActivity") as? Bool ?? true
            guard autoExpand else { return }
            autoCollapseTask?.cancel()

            if chatViewModel.expansionStage == 0 {
                // Background/proactive activity: peek with the status mini.
                chatViewModel.expansionStage = 1
                Task {
                    await dynamicNotch.expand()
                    isExpanded = true
        ...
        case .complete, .cancelled, .error, .idle:
            // Only auto-collapse the status mini (stage 1) that we opened for
            // background activity. If the user is in the full chat (stage 2) —
            // e.g. reading the answer — never yank it away.
            if twinState == .complete, chatViewModel.expansionStage == 1 {
                autoCollapseTask?.cancel()
    ...
    // `nonisolated deinit` intentionally omitted.
