...
final class AudioManager: NSObject {
    ...
    /// Called whenever TTS playback starts or stops (start, finish, stopTTS).
    /// Lets the UI show a live "Speaking…" status on the Dynamic Island.
    var onTTSStateChange: (() -> Void)?
    ...
    func stopTTS() {
        let wasPlaying = ttsPlayer?.isPlaying ?? false
        ttsPlayer?.stop()
        ttsPlayer = nil
        if wasPlaying {
            onTTSStateChange?()
