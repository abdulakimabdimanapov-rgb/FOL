...
// Used to bridge SwiftUI events back to AppDelegate for status logging.

extension Notification.Name {
    /// Posted when the full chat panel opens (expansionStage transitions to 2).
    /// object: nil
    static let secondSelfChatOpened = Notification.Name("com.secondself.chatOpened")

    /// Posted when the text input field gains focus.
    /// object: nil
    static let secondSelfInputFocused = Notification.Name("com.secondself.inputFocused")

    /// Posted when a message is sent by the user.
    /// object: nil
    static let secondSelfMessageSent = Notification.Name("com.secondself.messageSent")
}

// MARK: - Transition Tokens (inlined in views to avoid AnyTransition Sendable issues)

enum SSEEventType: String {
    case state
    case token
    /// Coarse, user-safe status from the Response Formatter — never contains
    /// tool names, arguments, or JSON.
    case activity
...
enum ServerConfig {
    ...
    /// FOL REST API — launched by the app in local mode (fol/run_api_server.py).
    static let folPort = 8754
    static let orchestratorURL = "http://localhost:\(orchestratorPort)"
    static let folAPIURL = "http://localhost:\(folPort)"
    ...
    /// Local speech-to-text (mlx-whisper on Apple Silicon) — no API key needed.
    static let folHealthEndpoint = "\(folAPIURL)/health"
    static let folSTTEndpoint = "\(folAPIURL)/api/stt"
}
