...
struct A2UIRenderer: View {
    ...
    private func componentView(for component: A2UIComponent) -> some View {
        ...
        case A2UIComponentType.actionInfo:
            if let data = ActionInfoData(from: component) {
                ActionInfoCard(
        ...
        case A2UIComponentType.peekNotification:
            if let data = PeekNotificationData(from: component) {
        ...
        case A2UIComponentType.riskConfirm:
            if let data = RiskConfirmData(from: component) {
                RiskConfirmCard(
                    ...
                    onAllow: {
                        onAction("allow", "Allowed: \(data.toolName)")
                    },
                    onDeny: {
                        onAction("deny", "Denied: \(data.toolName)")
                    }
                )
            } else {
                fallbackView(for: component)
        ...
        case A2UIComponentType.strictModal:
            if let data = StrictModalData(from: component) {
                StrictModalCard(
                    actionId: data.actionId,
                    toolName: data.toolName,
                    command: data.command,
                    onAllow: {
                        onAction("allow", "Executed: \(data.toolName)")
                    },
                    onDeny: {
                        onAction("deny", "Cancelled: \(data.toolName)")
                    }
                )
            } else {
                fallbackView(for: component)
