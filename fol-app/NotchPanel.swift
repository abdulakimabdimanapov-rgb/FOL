    ...
    var body: some View {
        HStack(spacing: 8) {
            if !viewModel.statusText.isEmpty {
                // Live status preview replaces the clock while the assistant is
                // listening / thinking / working / speaking (voice loop too)
                Text(viewModel.statusText)
                    .font(.system(size: 13, weight: .semibold))
                    .foregroundColor(.white)
                    .lineLimit(1)
                    .id("compact-status-\(viewModel.statusText)")
        ...
        .animation(.spring(response: 0.3, dampingFraction: 0.8), value: viewModel.statusText)
    }
}
...
struct DynamicIslandStatusBar: View {
    ...
    private var hasStatus: Bool {
        isActive || !viewModel.statusText.isEmpty
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 12) {
                ...
                if hasStatus {
                    if !viewModel.statusText.isEmpty {
                        // Real sanitized status — SSE activity, voice loop
                        // (Listening…/Transcribing…/Speaking…) or task states
                        Text(viewModel.statusText)
                            .font(.system(size: 14, weight: .medium))
                            .foregroundColor(.white.opacity(0.85))
                            .lineLimit(1)
                            .id("status-\(viewModel.statusText)")
        ...
        .frame(width: 360, height: hasStatus ? 56 : 44)
        .background(DynamicIslandBackground())
...
struct AIStatusDot: View {
    ...
    private var dotColor: Color {
        ...
        case .cancelled: return Color(hex: 0x8E8E93)
